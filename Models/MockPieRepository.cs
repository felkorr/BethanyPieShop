...
{
    public class MockPieRepository : IPieRepository//chama mock porque vamos usar dados simulados(mock)
    {

        private List<Pie> _pies;

        public MockPieRepository()//construtor que ira inicializar a lista caso esteja nula
        {
            if (_pies == null)
            {
                InitializePies();//chama o metodo abaixo para inicializar a lista com os dados abaixo
            }
        }
        ...
        {
            return _pies;//retorna a lista de tortas completas
        }
        ...
        {
            return _pies.FirstOrDefault(p => p.Id == pieId);//retorna a primeira torta que coincide com o pieId passado como parametro
        }
    }
}
